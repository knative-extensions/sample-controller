...
type AddressableServiceLister interface {
	...
	List(selector labels.Selector) (ret []*samples.AddressableService, err error)
...
// List lists all AddressableServices in the indexer.
func (s *addressableServiceLister) List(selector labels.Selector) (ret []*samples.AddressableService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*samples.AddressableService))
	})
	return ret, err
}
...
type AddressableServiceNamespaceLister interface {
	...
	Get(name string) (*samples.AddressableService, error)
...
// List lists all AddressableServices in the indexer for a given namespace.
func (s addressableServiceNamespaceLister) List(selector labels.Selector) (ret []*samples.AddressableService, err error) {
...
// Get retrieves the AddressableService from the indexer for a given namespace and name.
func (s addressableServiceNamespaceLister) Get(name string) (*samples.AddressableService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(samples.Resource("addressableservice"), name)
	}
	return obj.(*samples.AddressableService), nil
}
